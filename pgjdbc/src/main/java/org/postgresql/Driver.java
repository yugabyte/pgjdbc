  ...
  private static Connection getConnectionBalanced(LoadBalanceProperties lbprops) {
    ...
    if (chosenHost == null) {
      ...
      try {
        ...
        controlConnection = new PgConnection(hspec, props, url);
  ...
  public static @Nullable Properties parseURL(String url, @Nullable Properties defaults) {
    ...
    if (defaults != null) {
      if (defaults.containsKey("load-balance")) {
        priority1Url.setProperty("load-balance", defaults.getProperty("load-balance"));
      }
      if (defaults.containsKey("topology-keys")) {
        priority1Url.setProperty("topology-keys", defaults.getProperty("topology-keys"));
      }
    }
